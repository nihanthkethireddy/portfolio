...
export default function Experience() {
  ...
  return <section id="experience" className="section"><header className="section-heading"><span>02 / EXPERIENCE</span><h2>Building across the<br /><em>full product stack.</em></h2></header>
    <div className="timeline">{EXPERIENCES.map((item, index) => { const open = expanded === index; return <motion.article className={`experience-card ${open ? 'expanded' : ''}`} key={`${item.company}-${item.dates}`} style={{ '--company-accent': item.accent } as React.CSSProperties} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
